...
class Gaussian_Naive_Bayes():
    ...
    def gen_by_class(self):
        """
        With the gfiven input dataset (self.x, self.y), generate 3 dictionaries to calculate class-wise mean and variance of the data.
    ...
    def predict(self, x):
        ...
        print("PREDICTING ")
        prediction = 1/(self.std*math.sqrt(2*math.pi))*np.exp((-1/2)*math.pow((x-self.mean(x)/x-self.std(x),2)))
        # END_YOUR_CODE
        ############################################################
        ############################################################
...
class Neural_Network():
    ...
    def loss(self, x_batch, y_batch, reg = 1e-3):
            ...
            ############################################################
            ############################################################
            return loss, gradient  #uncomment me later, Ziro!
